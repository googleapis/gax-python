...
def _parse_template(template):
  ...
  for seg in map(str.strip, template.split('/')):
    ...
    if seg == '**' or seg == '*':
      ...
      wildcard = _PATH_WILDCARD_SEGMENT if len(seg) == 2 else _WILDCARD_SEGMENT
      if var_name is None:
        segment_list.append(
    ...
    if path_wildcard_bound > 1:
      raise ValidationException(
          'parse error: pattern must not contain more than one path wildcard '
          '(\'**\') in \'%s\'' % template)
  if custom_verb:
...
class PathTemplate(object):
    ...
    for seg in self.segments:
      if seg.kind == SegmentKind.BINDING:
        if seg.value in bindings:
          raise ValidationException('duplicate binding \'%s\'', seg.value)
  ...
  def match(self, path):
    ...
    All matched values will be properly unescaped using URL encoding rules. If
    the path does not match the template, None is returned.
  ...
  def match_from_full_name(self, path):
    ...
    Matches the path, where the first segment is interpreted as the host name
    regardless of whether it starts with '//' or not.
  ...
  def _match(self, path, force_host_name):
    ...
    if last.kind == SegmentKind.CUSTOM_VERB:
      ...
      if not match_obj or (self._decode_url(match_obj.group(1)) != last.value):
        return None
      ...
      if seg.kind == SegmentKind.END_BINDING:
        ...
        if seg.kind == SegmentKind.PATH_WILDCARD:
          ...
          for i in range(seg_pos, len(self.segments)):
            if (self.segments[i].kind == SegmentKind.BINDING or
                self.segments[i].kind == SegmentKind.END_BINDING):
              continue
            else:
              segs_to_match += 1
  ...
  def instantiate(self, *args):
    ...
    wildcards in the template must have bindings of '$n' variables, where 'n'
    is the position of the wildcard (starting at 0).
  ...
  def instantiate_partial(self, values):
    ...
    Similar to instantiate(*args), but allows for unbound variables, which are
    substituted with their original syntax.

    Args:
      values: (dict) A dictionary of binding pairs.

    Returns:
      (str) An instantiated path template string. Can be used to create a new
        template.
  ...
  def _instantiate(self, values, allow_partial):
    ...
    result = ''
    if values.has_key(self.HOSTNAME_VAR):
    ...
    for i in iterator:
      ...
      if seg.kind == SegmentKind.BINDING:
        var = seg.value
        if not values.has_key(seg.value):
          if not allow_partial:
            raise ValidationException(
                'unbound variable \'%s\'. bindings: %s' % (var, values))
          if var.startswith('$'):
