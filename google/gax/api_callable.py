...
def _retryable(call, max_attempts):
  ...
  Args:
    call: A function.
    max_attempts: The maximum number of times that the call should be attempted;
      the call will always be attempted at least once.
...
def _page_streaming(
    call, request, timeout, request_page_token_field, response_page_token_field,
    resource_field):
  """Creates an iterable that performs page streaming for a gRPC call.

  Args:
    call: A gRPC call taking a proto request and a timeout whose return type
      contains a field for a page token and a repeated field representing a
      resource.
    request: The proto request to the gRPC call.
    timeout: The timeout for the call.
    request_page_token_field: The field of the page token in the request proto.
...
def _page_streamable(
    call, request_page_token_field, response_page_token_field, resource_field):
  ...
  Args:
    ...
    response_page_token_field: The field of the next page token in the response.
...
class ApiCallableDefaults(object):
  ...
  def __init__(self, timeout=30, is_idempotent_retrying=True, max_attempts=16):
    """Constructor.

    Args:
      timeout: The client-side timeout for gRPC calls.
      is_idempotent_retrying: If set, calls determined by configuration to be
        idempotent will retry upon transient error by default.
      max_attempts: The maximum number of attempts that should be made for a
        retrying call to this service.
...
def idempotent_callable(
    func, timeout=None, is_retrying=None, page_streaming=None,
    max_attempts=None, defaults=None):
  ...
  Args:
    ...
    is_retrying: Boolean indicating whether this call should retry upon a
      transient error. If None, retrying will be determined by the defaults
      parameter.
    page_streaming: page_descriptor.PageDescriptor indicating the structure
      of page streaming to be performed. If None, this call will not perform
      page streaming.
    max_attempts: If is_retrying, the maximum number of times this call may
      ...
      defaults parameter.
    defaults: An ApiCallableDefaults object, from which default values will be
      drawn if not supplied by the other named parameters. The other named
      parameters always override those in the defaults. If neither the
      is_retrying nor defaults parameter is specified, a runtime error will
      result at callable creation time.
  ...
  return ApiCallable(
      ...
      max_attempts=max_attempts, defaults=defaults,
      is_retrying=to_retry)
...
class ApiCallable(object):
  ...
  """
  def __init__(
      self, func, timeout=None, is_retrying=False, page_streaming=None,
      max_attempts=None, defaults=None):
    """Constructor.
    ...
    Args:
      ...
      defaults: An ApiCallableDefaults object, from which default values will be
        drawn if not supplied by the other named parameters. The other named
        parameters always override those in the defaults. If neither the
        defaults nor timeout parameter is specified, a runtime error will
        result at call time. If neither the defaults nor the max_attempts
        parameter is specified for a retrying call, a runtime error will
        result at call time.
